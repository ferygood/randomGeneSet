...
hmTEexp <- hmTE %>% select(-c(1,2,3))
rownames(hmTEexp) <- hmTE$name

# genes covert to tpm
sample_counts <- colSums(df_hm_gene)
scaling_factor <- sample_counts / 1e6
df_hm_gene_tpm <- t(t(df_hm_gene)/ scaling_factor + 1) * 1e6
df_hm_gene_tpm <- as.data.frame(df_hm_gene_tpm)

# tes convert to tpm
te_counts <- colSums(hmTEexp)
te_scale <- te_counts / 1e6
hmTE_tpm <- t(t(hmTEexp)/ scaling_factor + 1) * 1e6
...
c1_id <- metadata %>%
    filter(Organism=="Homo sapiens" & brain_region %in% c1_region$brain_region) %>%
    select(1)

c2_id <- metadata %>%
    filter(Organism=="Homo sapiens" & brain_region %in% c2_region$brain_region) %>%
    select(1)
...
for (i in 1:1000){
    ...
    file_c1 <- paste0("results_c1_hm/gene_", i, "_vs_TE_corr.csv")
    ...
    file_c2 <- paste0("results_c2_hm/gene_", i, "_vs_TE_corr.csv")
...
write.csv(df_temp_c1, file="hm_c1_result.csv", row.names=F)
write.csv(df_temp_c2, file="hm_c2_result.csv", row.names=F)
